...
#include <QtPrintSupport/QPrintDialog>   // if you use QPrintDialog
#include <QPainter>
    ...
    , currentPageIndex(-1)
{
    ...
    // Connect sponsor UI buttons to slots (from sponsorWidget via findChild)
    if (sponsorWidget) {
        connect(sponsorWidget->findChild<QPushButton*>("addButton"), &QPushButton::clicked, this, &MainWindow::addSponsor);
        connect(sponsorWidget->findChild<QPushButton*>("updateButton"), &QPushButton::clicked, this, &MainWindow::updateSponsor);
        connect(sponsorWidget->findChild<QPushButton*>("deleteButton"), &QPushButton::clicked, this, &MainWindow::deleteSelectedSponsor);
        connect(sponsorWidget->findChild<QPushButton*>("afficherButton"), &QPushButton::clicked, this, &MainWindow::loadSponsoring);
        connect(sponsorWidget->findChild<QPushButton*>("search1_2"), &QPushButton::clicked, this, &MainWindow::searchSponsorById);
        connect(sponsorWidget->findChild<QPushButton*>("pdfexport"), &QPushButton::clicked, this, &MainWindow::on_pdfexport_clicked);
        connect(sponsorWidget->findChild<QPushButton*>("sort"), &QPushButton::clicked, this, &MainWindow::on_sort_clicked);
        connect(sponsorWidget->findChild<QPushButton*>("clear"), &QPushButton::clicked, this, &MainWindow::clearFields);
        connect(sponsorWidget->findChild<QTableWidget*>("affiche"), &QTableWidget::clicked, this, &MainWindow::onSponsorSelected);
        connect(sponsorWidget->findChild<QPushButton*>("Stat"), &QPushButton::clicked, this, &MainWindow::on_pushButton_stat_clicked);
        ...
        connect(sponsorWidget->findChild<QPushButton*>("calendrier"), &QPushButton::clicked, this, &MainWindow::openCalendar);
    }
...
// =============================================================================
// SPONSOR MANAGEMENT IMPLEMENTATION
// =============================================================================
...
void MainWindow::addSponsor() {
    // Get sponsor data from UI fields
    Sponsor sponsor = getSponsorFromFields();

    // Validate and add to database
    QString errorMsg;
    if (Sponsor::add(sponsor, errorMsg)) {
        ...
        QString body = "Bonjour " + sponsor.getNom() + ",\n\n"
                                                       "Merci d'avoir rejoint nos sponsors.\n"
                                                       "Cordialement.";

        if (smtp.sendMail(sponsor.getEmail(), subject, body)) {
...
void MainWindow::loadSponsoring() {
    ...
    QList<Sponsor> sponsors = Sponsor::getAll();

    // Clear and setup table
    sponsorWidget->getAfficheTable()->clear();
    sponsorWidget->getAfficheTable()->setRowCount(0);
    sponsorWidget->getAfficheTable()->setColumnCount(6);

    QStringList headers = {"ID", "Nom", "Email", "Date Début", "Date Fin", "Status"};
    sponsorWidget->getAfficheTable()->setHorizontalHeaderLabels(headers);

    // Populate table with sponsors
    for (int i = 0; i < sponsors.size(); ++i) {
        sponsorWidget->getAfficheTable()->insertRow(i);
        const Sponsor &s = sponsors[i];

        sponsorWidget->getAfficheTable()->setItem(i, 0, new QTableWidgetItem(QString::number(s.getId())));
        sponsorWidget->getAfficheTable()->setItem(i, 1, new QTableWidgetItem(s.getNom()));
        sponsorWidget->getAfficheTable()->setItem(i, 2, new QTableWidgetItem(s.getEmail()));
        sponsorWidget->getAfficheTable()->setItem(i, 3, new QTableWidgetItem(s.getDateDebut().toString()));
        sponsorWidget->getAfficheTable()->setItem(i, 4, new QTableWidgetItem(s.getDateFin().toString()));
        sponsorWidget->getAfficheTable()->setItem(i, 5, new QTableWidgetItem(s.getStatus()));
    }

    sponsorWidget->getAfficheTable()->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
}

void MainWindow::deleteSelectedSponsor() {
    // Check if a row is selected
    int row = sponsorWidget->getAfficheTable()->currentRow();
    if (row < 0) {
    ...
    int id = sponsorWidget->getAfficheTable()->item(row, 0)->text().toInt();
...
void MainWindow::updateSponsor()
{
    ...
    QString errorMsg;
    if (Sponsor::update(sponsor, errorMsg)) {
        QMessageBox::information(this, "Succès", "Sponsor mis à jour avec succès.");
        loadSponsoring();
        sponsorWidget->getIDLineEdit()->setDisabled(false); // Re-enable ID field
    } else {
        QMessageBox::critical(this, "Erreur", errorMsg);
    }
}
...
{
    QString searchId = sponsorWidget->getSearchLineEdit()->text().trimmed();
    ...
    // Search for sponsors by ID
    QList<Sponsor> sponsors = Sponsor::search("ID_SPONS", searchId);
...
void MainWindow::on_sort_clicked()
{
    ...
    int row = 0;
    while (query.next()) {
        sponsorWidget->getAfficheTable()->insertRow(row);
        sponsorWidget->getAfficheTable()->setItem(row, 0, new QTableWidgetItem(query.value("ID_SPONS").toString()));
        sponsorWidget->getAfficheTable()->setItem(row, 1, new QTableWidgetItem(query.value("NOM_SPONS").toString()));
        sponsorWidget->getAfficheTable()->setItem(row, 2, new QTableWidgetItem(query.value("EMAIL").toString()));
        sponsorWidget->getAfficheTable()->setItem(row, 3, new QTableWidgetItem(query.value("DATE_DEBUT").toString()));
        sponsorWidget->getAfficheTable()->setItem(row, 4, new QTableWidgetItem(query.value("DATE_FIN").toString()));
        sponsorWidget->getAfficheTable()->setItem(row, 5, new QTableWidgetItem(query.value("STATUS").toString()));
        row++;
    }
...
{
    sponsorWidget->getIDLineEdit()->clear();
    sponsorWidget->getNomLineEdit()->clear();
    sponsorWidget->getEmailLineEdit()->clear();
    sponsorWidget->getDateDebutEdit()->setDate(QDate::currentDate());
    sponsorWidget->getDateFinEdit()->setDate(QDate::currentDate());
    sponsorWidget->getStatusComboBox()->setCurrentIndex(-1); // deselects combo box
    sponsorWidget->getIDLineEdit()->setDisabled(false);   // re-enable ID if it was locked
}
...
{
    // Get the sponsor ID from the selected row
    int row = index.row();
    int id = sponsorWidget->getAfficheTable()->item(row, 0)->text().toInt();

    // Load sponsor from database
    bool found;
    Sponsor sponsor = Sponsor::getById(id, found);

    if (found) {
        populateFieldsFromSponsor(sponsor);
        sponsorWidget->getIDLineEdit()->setDisabled(true);
    } else {
...
QMap<QString,int> MainWindow::getStatusFromTable()
{
    ...
    int rows = sponsorWidget->getAfficheTable()->rowCount();
    int statusCol = 5; // index of STATUS column

    for (int i = 0; i < rows; ++i) {
        QTableWidgetItem *item = sponsorWidget->getAfficheTable()->item(i, statusCol);
        if (!item) continue;
...
void MainWindow::drawPieChart(const QMap<QString,int>& stats)
{
    ...
    for (auto it = stats.begin(); it != stats.end(); ++it) {
        ...
        row->setText(QString("%1 — %2 (%3%)").arg(name).arg(count).arg(QString::number(pct, 'f', 1)));
        row->setMargin(4);
...
// Helper methods
Sponsor MainWindow::getSponsorFromFields() {
    int id = sponsorWidget->getIDLineEdit()->text().toInt();
    QString nom = sponsorWidget->getNomLineEdit()->text();
    QString email = sponsorWidget->getEmailLineEdit()->text();
    QDate dd = sponsorWidget->getDateDebutEdit()->date();
    QDate df = sponsorWidget->getDateFinEdit()->date();
    QString status = sponsorWidget->getStatusComboBox()->currentText();

    return Sponsor(id, nom, email, dd, df, status);
}


void MainWindow::populateFieldsFromSponsor(const Sponsor &sponsor) {
    sponsorWidget->getIDLineEdit()->setText(QString::number(sponsor.getId()));
    sponsorWidget->getNomLineEdit()->setText(sponsor.getNom());
    sponsorWidget->getEmailLineEdit()->setText(sponsor.getEmail());
    sponsorWidget->getDateDebutEdit()->setDate(sponsor.getDateDebut());
    sponsorWidget->getDateFinEdit()->setDate(sponsor.getDateFin());
    sponsorWidget->getStatusComboBox()->setCurrentText(sponsor.getStatus());
}
