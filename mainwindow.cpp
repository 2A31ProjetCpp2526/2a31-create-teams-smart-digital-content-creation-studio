    ...
    , projectWidget(nullptr)
{
    ...
    connect(projectWidget, &ProjectWidget::deleteProjectRequested,
            this, &MainWindow::onProjectDeleteRequested);
    connect(projectWidget, &ProjectWidget::editProjectRequested,
            this, &MainWindow::onProjectEditRequested);
    connect(projectWidget, &ProjectWidget::addProjectRequested,
            this, &MainWindow::onProjectAddRequested);
    ...
    // Set Employer button as active on startup
...
void MainWindow::onProjectAddRequested(const QString &pid,
                                       const QString &titre,
                                       ...
                                       const QString &status)
{
    ...
    if (titre.trimmed().isEmpty() || description.trimmed().isEmpty() || status.trimmed().isEmpty()) {
        QMessageBox::warning(this, "Erreur de saisie",
                             "Veuillez remplir tous les champs requis (titre, description, statut).");
        return;
    }

    // Check that TITRE, DESCRIPTION, STATUS are alphabetic strings (letters + spaces)
    QRegularExpression stringRegex("^[A-Za-zÀ-ÖØ-öø-ÿ\\s]+$");
    if (!stringRegex.match(titre).hasMatch()) {
        QMessageBox::warning(this, "Erreur de saisie",
                             "Le titre doit contenir uniquement des lettres et des espaces.");
        return;
    }
    if (!stringRegex.match(description).hasMatch()) {
        QMessageBox::warning(this, "Erreur de saisie",
                             "La description doit contenir uniquement des lettres et des espaces.");
        return;
    }
    if (!stringRegex.match(status).hasMatch()) {
        QMessageBox::warning(this, "Erreur de saisie",
                             "Le statut doit contenir uniquement des lettres et des espaces.");
        return;
    }

    // If PID is not empty, verify it’s numeric
    if (!pid.trimmed().isEmpty()) {
        bool ok;
        int idVal = pid.toInt(&ok);
        if (!ok || idVal <= 0) {
            QMessageBox::warning(this, "Erreur de saisie",
                                 "L'identifiant du projet doit être un nombre positif.");
            return;
        }
    }
    ...
    if (pid.isEmpty()) {
        query.prepare("INSERT INTO PROJET (TITRE, DESCRIPTION, STATUS, DATE_CREATION) "
                      "VALUES (:titre, :description, :status, :date)");
        ...
        query.prepare("INSERT INTO PROJET (PROJET_ID, TITRE, DESCRIPTION, STATUS, DATE_CREATION) "
                      "VALUES (:id, :titre, :description, :status, :date)");
        query.bindValue(":id", pid.toInt());
        query.bindValue(":titre", titre);
        query.bindValue(":description", description);
        query.bindValue(":status", status);
        query.bindValue(":date", QDate::currentDate());
    ...
    if (success) {
        ...
        qDebug() << "✅ Project inserted successfully";

        // Clear form fields
        if (projectWidget) {
            projectWidget->findChild<QLineEdit*>("pid")->clear();
            projectWidget->findChild<QLineEdit*>("titre")->clear();
            projectWidget->findChild<QLineEdit*>("desc")->clear();
            QComboBox *cb = projectWidget->findChild<QComboBox*>("status");
            if (cb) cb->setCurrentIndex(0);
        }
    } else {
        QString err = query.lastError().text();
        QMessageBox::critical(this, "Erreur", "Échec d’ajout du projet : " + err);
        qDebug() << "❌ Insert failed:" << err;
    }
}

void MainWindow::onProjectEditRequested(const QString &pid,
                                        const QString &titre,
                                        const QString &description,
                                        const QString &status)
{
    // 🔧 Example logic — replace with your actual data handling

    // If you’re showing projects in a QTableWidget:
    /*
    for (int row = 0; row < ui->projectTable->rowCount(); ++row) {
        if (ui->projectTable->item(row, 0)->text() == pid) {
            ui->projectTable->item(row, 1)->setText(titre);
            ui->projectTable->item(row, 2)->setText(description);
            ui->projectTable->item(row, 3)->setText(status);
            break;
        }
    }
    */

    qDebug() << "Edited project:" << pid << titre << description << status;
}
void MainWindow::onProjectDeleteRequested(QString pid)
{
    qDebug() << "Deleted project with ID:" << pid;

    // Optional: refresh your table or project list if you have one
    // Example:
    // loadProjects();
}
